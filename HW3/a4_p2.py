...
def fitPredictorRBF(x,y,k):
    ...
    gammaList = [gamma0/z for z in range(1,10)] +\
                          [gamma0*z for z in range(2,10)]
    ...
    for j,params in enumerate(paramsComb):
        ...
        foldSize = x.shape[0]//10
        for i in range(10):
            foldInd = int(i * foldSize)
            
            # split into train/test sets
            xTrain,xTest = np.delete(x,np.s_[foldInd:foldInd+foldSize])[:,None],x[foldInd:foldInd+foldSize][:,None]
            yTrain,yTest = np.delete(y,np.s_[foldInd:foldInd+foldSize])[:,None],y[foldInd:foldInd+foldSize][:,None]
                
            # get a predictor function for the fit model
            f = getPredictor(xTrain, yTrain, lambda x,y : RBF(x,y,gamma), lambda_reg)
            
            sqErrArray[i] = np.linalg.norm((f(xTest)-yTest).squeeze(),ord=2)**2
...
def fitPredictorPoly(x,y,k):
    ...
    # get a predictor function for the fit model
    fStar = getPredictor(x,y, lambda x,y : Poly(x,y,d),lambda_reg)
...
def runKernelRegression(n):
    ...
    x,yTrue,y = generateData(n)
    f_rbf,lambda_reg_rbf,gamma = fitPredictorRBF(x, y, RBF)
    
    print('RBF kernel:')
    print('lambda: {}'.format(lambda_reg_rbf)) #1e-05
    print('gamma: {}'.format(gamma)) #22.703978968770407
    
    # Fit params for polynomial kernel
    f_poly,lambda_reg_poly,d = fitPredictorPoly(x, y, Poly)
    
    print('Polynomial kernel:')
    print('lambda: {}'.format(lambda_reg_poly)) #1e-08
    print('d: {}'.format(d)) #20
    ...
    bootstrapInds = np.random.choice(range(n),size=(B,n))
    ...
    m=1000
    newX,newTrueY,newY = generateData(m)
    bootstrapDiff = np.zeros((B,1))
    
    bootstrapInds = np.random.choice(range(m),size=(B,m))
    x_b,y_b = newX[bootstrapInds],newY[bootstrapInds]
    
    for b in range(B):
        errRbf = np.mean((f_rbf(x_b[b])-y_b[b])**2)
        errPoly = np.mean((f_poly(x_b[b])-y_b[b])**2)
        bootstrapDiff[b] = errPoly - errRbf
        
    diffQuantileL = np.quantile(bootstrapDiff,low) #0.11956895343420021
    diffQuantileH = np.quantile(bootstrapDiff,high) #0.22555011642050002
    
    #Since 95% CI does not contain zero, we can conlude that the regression 
    # function from the RBF kernel is better with 95% confidence
    
    print('5% quantile: {}'.format(diffQuantileL))
    print('95% quantile: {}'.format(diffQuantileH))
    
n = 300
runKernelRegression(n)
